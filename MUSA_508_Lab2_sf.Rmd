...
First, we can see what `class` the `acsTractsPHL.2016.sf` object is. It is class `sf` first and then `data.table` second. This means that you can write R code to treat this as a spatial class, but that you can also do a lot of the things you would normally do with a `data.table` to it as well. That is very handy!
...
Combining the points with out ACS tract outlines works since both objects have the same CRS of `4326`.
...
To remove the geometry all together is as easy as using `st_drop_geometry()`. This leaves you with a plain data.frame and as aspects of the spatial data, including the geometry column are gone.
...
Spatial joins are at the foundation of doing spatial analysis. Typical table joins allow us to columns based on an identifier. We used that approach above and used `st_as_af()` to make it spatial. This works well for point coordinates and when the filtering is not spatial. In cases where the filter, sorting, and summarizing operations are spatial we need spatial joins. This exercise ends on a quick spatial join example, but the following exercises and assignments require spatial operations like this to replicate typical GIS functionality.
